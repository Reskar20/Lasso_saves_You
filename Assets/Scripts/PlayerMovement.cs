...
//			Debug.Log("on jump input");
	...
	//		Debug.Log("on jump up input");
		...
		{
			//Ground Check
			if (Physics2D.OverlapBox(_groundCheckPoint.position, _groundCheckSize, 0, _groundLayer) && IsJumping == false) //checks if set box overlaps with ground
			{
				LastOnGroundTime = Data.coyoteTime; //if so sets the lastGrounded to coyoteTime
            }		

			//Right Wall Check
			if (((Physics2D.OverlapBox(_frontWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && IsFacingRight == true)
					|| (Physics2D.OverlapBox(_backWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && IsFacingRight == false)) && IsWallJumping == false)
				LastOnWallRightTime = Data.coyoteTime;

			//Right Wall Check
			if (((Physics2D.OverlapBox(_frontWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && IsFacingRight == false)
				|| (Physics2D.OverlapBox(_backWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && IsFacingRight == true)) && IsWallJumping== false)
		...
		if (IsWallJumping == true && Time.time - _wallJumpStartTime > Data.wallJumpTime)
		{
			IsWallJumping = false;
		}
		...
		//Jump
		if (CanJump() == true && LastPressedJumpTime > 0)
		{
			IsJumping = true;
			IsWallJumping = false;
			_isJumpCut = false;
		...
		//WALL JUMP
		else if (CanWallJump() == true && LastPressedJumpTime > 0)
		{
			IsWallJumping = true;
			IsJumping = false;
			_isJumpCut = false;
...
*/
		...
		else if (_isJumpCut == true)
		{
			//Higher gravity if jump button released
			SetGravityScale(Data.gravityScale * Data.jumpCutGravityMult);
			RB.linearVelocity = new Vector2(RB.linearVelocity.x, Mathf.Max(RB.linearVelocity.y, -Data.maxFallSpeed));
		}
		else if ((IsJumping == true || IsWallJumping == true || _isJumpFalling == true) && Mathf.Abs(RB.linearVelocity.y) < Data.jumpHangTimeThreshold)
		{
	...
	{
		if (CanJumpCut() == true|| CanWallJumpCut() == true)
		{
			_isJumpCut = true;
			Debug.Log("jump cut");			
		}
	...
	private void Run(float lerpAmount)
	{
		...
		if ((IsJumping == true || IsWallJumping == true || _isJumpFalling == true) && Mathf.Abs(RB.linearVelocity.y) < Data.jumpHangTimeThreshold)
		{
